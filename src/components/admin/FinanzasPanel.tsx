...
export default function FinanzasPanel({ terminoBusqueda, abrirModal }: FinanzasPanelProps) {
    ...
    <div className="space-y-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-azul-primario mt-2">
                {resumenFinanciero.ingresosMes.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
              </h3>
            </div>
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-azul-primario mt-2">
                {resumenFinanciero.gastosMes.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
              </h3>
            </div>
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-azul-primario mt-2">
                {resumenFinanciero.pagosAbogadosMes.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
              </h3>
            </div>
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-azul-primario mt-2">
                {resumenFinanciero.balanceMes.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
              </h3>
            </div>
              ...
              {transaccionesFiltradas.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-4 text-center text-gray-500">
                    No se encontraron transacciones con los criterios de búsqueda.
                  </td>
                </tr>
              ) : (
                transaccionesFiltradas.map((transaccion) => (
                  <tr key={transaccion.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{transaccion.concepto}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        ...
                        {transaccion.abogado && (
                          <div className="flex items-center">
                            <FiUserCheck className="text-gray-400 mr-1" size={14} />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {transaccion.metodoPago ? (
                          <div className="flex items-center">
                            <FiCreditCard className="text-gray-400 mr-1" size={14} />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        ...
                        {transaccion.estado === 'pendiente' && (
                          <button
                            onClick={() => cambiarEstadoTransaccion(transaccion.id, 'completado')}
                        ...
                        {transaccion.estado === 'pendiente' && (
                          <button
                            onClick={() => cambiarEstadoTransaccion(transaccion.id, 'cancelado')}
      ...
      {transacciones.filter(t => t.estado === 'pendiente').length > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <h3 className="text-lg font-medium text-yellow-800 mb-2">Transacciones pendientes</h3>
          <p className="text-sm text-yellow-700">
            Tienes {transacciones.filter(t => t.estado === 'pendiente').length} transacciones pendientes por un valor total de {
              transacciones
