...
import { FiAlertCircle, FiCreditCard, FiRefreshCw, FiArrowLeft, FiHelpCircle } from 'react-icons/fi';
...
export default function ErrorPagoPage() {
  ...
  const [errorMessage, setErrorMessage] = useState<string>("Ha ocurrido un problema al procesar tu pago");
  ...
  useEffect(() => {
    ...
    if (code) {
      ...
      if (code === 'insufficient_funds') {
        setErrorMessage('No se ha podido completar el pago por fondos insuficientes');
  ...
  const solicitarAyuda = () => {
    ...
    router.push('/contacto?motivo=error_pago&codigo=' + (errorCode || 'desconocido'));
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        >
          <div className="bg-red-50 p-8 text-center border-b border-red-100">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">No se ha podido completar el pago</h1>
            <p className="text-gray-600">
              {errorMessage}
              {errorCode && <span className="block text-sm mt-1 text-gray-500">Código de error: {errorCode}</span>}
            </p>
          </div>
          
          {serviceData && (
            <div className="p-4 bg-gray-50 border-b border-gray-200">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Servicio seleccionado:</h3>
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-gray-900">{serviceData.nombre}</p>
                  <p className="text-sm text-gray-600">{serviceData.descripcion}</p>
                </div>
                <p className="font-bold text-azul-primario">
                  {typeof serviceData.precio === 'number' 
                    ? serviceData.precio.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })
                    : serviceData.precio}
                </p>
              </div>
            </div>
          ...
          <div className="p-6">
            <div className="mb-8">
              ...
              <ul className="space-y-3 text-gray-600">
                ...
                <li className="flex items-start">
                  <FiCreditCard className="mt-1 mr-2 text-gray-400 flex-shrink-0" />
                  <span>La entidad bancaria ha rechazado la operación por motivos de seguridad</span>
                </li>
                <li className="flex items-start">
                  <FiCreditCard className="mt-1 mr-2 text-gray-400 flex-shrink-0" />
            ...
            <div className="mb-8">
              ...
              <ul className="space-y-3 text-gray-600">
                ...
                <li className="flex items-start">
                  ...
                  <span>Contactar con tu entidad bancaria para verificar si hay alguna restricción</span>
                </li>
              </ul>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-6">
              <p className="text-sm text-gray-600 italic">
                Si continúas teniendo problemas, por favor contacta con nuestro equipo de soporte a través del correo electrónico <span className="font-medium">soporte@virtuabogado.com</span> o llamando al <span className="font-medium">900 123 456</span>.
              </p>
            </div>
          </div>
        </motion.div>
